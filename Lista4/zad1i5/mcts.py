...
class Board:
    ...
    def Q(self, state):
        q = sqrt(1 * log(self.N) / state.playouts)
...
for i in range(rounds):
    game = Board(player)
    one_round = game.play_first()
    if( one_round > 0 ):
        max_victories += 1
    else:
        min_victories += 1
    player = 1 - player
    print("my victories: {} / {}".format(max_victories, i+1))
